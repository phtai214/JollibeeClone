...
{
    // Tăng giới hạn model binding cho combo với 10+ nhóm/options
    options.MaxModelBindingCollectionSize = 8192; // Tăng lên 8192 để support complex combos
})
...
{
    // Tăng giới hạn cho model binding - support 10+ groups with multiple options each
    options.MaxModelBindingCollectionSize = 8192; // Tăng lên 8192
    options.ModelBindingMessageProvider.SetValueMustNotBeNullAccessor(value => "Trường này là bắt buộc.");
});

// Cấu hình Form Options để hỗ trợ nhiều fields cho combo lớn
builder.Services.Configure<Microsoft.AspNetCore.Http.Features.FormOptions>(options =>
{
    options.ValueCountLimit = 12288; // Tăng lên 12288 cho combo với 10+ groups
    options.KeyLengthLimit = 8192; // Tăng key length cho complex field names
    options.ValueLengthLimit = 2 * 1024 * 1024; // 2MB cho large form values
    options.MultipartBodyLengthLimit = 256 * 1024 * 1024; // 256MB cho file uploads
    options.MultipartHeadersCountLimit = 2048; // Tăng headers count
    options.MultipartHeadersLengthLimit = 65536; // Tăng headers length (64KB)
});
