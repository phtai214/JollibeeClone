...
namespace JollibeeClone.Areas.Admin.ViewModels
{
    public class PromotionViewModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            // Business rule: Must have at least one product or category selected
            if (!SelectedProductIds.Any() && !SelectedCategoryIds.Any())
            {
                yield return new ValidationResult("Vui lòng chọn ít nhất một sản phẩm hoặc danh mục để áp dụng voucher", new[] { nameof(SelectedProductIds), nameof(SelectedCategoryIds) });
            }
        }
    }
