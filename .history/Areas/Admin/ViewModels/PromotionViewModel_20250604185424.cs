...
namespace JollibeeClone.Areas.Admin.ViewModels
{
    public class PromotionViewModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            // Business rule: Must have at least one product or category selected (flexible - can be either or both)
            // if (!SelectedProductIds.Any() && !SelectedCategoryIds.Any())
            // {
            //     yield return new ValidationResult("Vui lòng chọn ít nhất một sản phẩm HOẶC một danh mục để áp dụng voucher", new[] { nameof(SelectedProductIds), nameof(SelectedCategoryIds) });
            // }
        }
    }
