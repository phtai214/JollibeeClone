            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (var groupViewModel in viewModel.ConfigGroups)
                    {
                        ...
                        {
                            // Validation: ProductID phải được chọn
                            if (optionViewModel.ProductID <= 0)
                            {
                                ModelState.AddModelError("", $"Nhóm '{groupViewModel.GroupName}': Vui lòng chọn sản phẩm cho tất cả các option.");
                                throw new InvalidOperationException("ProductID không hợp lệ");
                            }

                            // Validation: VariantID phải được chọn (vì tất cả sản phẩm đều có variants)
                            if (!optionViewModel.VariantID.HasValue || optionViewModel.VariantID.Value <= 0)
                            {
                                ModelState.AddModelError("", $"Nhóm '{groupViewModel.GroupName}': Vui lòng chọn biến thể cho tất cả các option.");
                                throw new InvalidOperationException("VariantID không hợp lệ");
                            }
                            ...
                            var option = new ProductConfigurationOption
                            {
                                ...
                                VariantID = optionViewModel.VariantID.Value, // Now we're sure it's not null
