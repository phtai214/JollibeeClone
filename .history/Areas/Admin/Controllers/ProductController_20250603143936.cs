...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("ProductID,ProductName,ShortDescription,Price,OriginalPrice,CategoryID,IsConfigurable,IsAvailable,ImageUrl")] Product product, IFormFile? imageFile)
        {
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    if (imageFile != null && imageFile.Length > 0)
                    {
                        ...
                        product.ImageUrl = $"/uploads/products/{fileName}";
                        _logger.LogInformation("New image uploaded: {ImageUrl}", product.ImageUrl);
                    }
                    else
                    {
                        // Keep existing image
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        _logger.LogInformation("Product updated successfully: {ProductName}. ImageUrl: {ImageUrl}", 
                            product.ProductName, product.ImageUrl ?? "NULL");
