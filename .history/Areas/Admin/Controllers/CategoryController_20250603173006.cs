...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        // GET: Admin/Category
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber, string view = "grid")
        {
            try
            {
                ...
                switch (sortOrder)
                {
                    ...
                    case "order_desc":
                        ...
                        break;
                    case "status":
                        categories = categories.OrderBy(c => c.IsActive);
                        break;
                    case "status_desc":
                    ...
                    case "date_desc":
                        ...
                        break;
                    default:
                        categories = categories.OrderBy(c => c.DisplayOrder).ThenBy(c => c.CategoryName);
                        break;
                }

                int pageSize = 10;
                var paginatedCategories = await PaginatedList<Categories>.CreateAsync(categories.AsNoTracking(), pageNumber ?? 1, pageSize);
            ...
            catch (Exception ex)
            {
                ...
                return View(new PaginatedList<Categories>(new List<Categories>(), 0, 1, 10));
            }
        }
