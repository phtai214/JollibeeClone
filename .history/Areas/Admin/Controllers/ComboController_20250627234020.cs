...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class ComboController : Controller
    {
        ...
        public async Task<IActionResult> Create(ComboCreateViewModel viewModel)
        {
            ...
            var totalFields = Request.Form.Count;
            System.Diagnostics.Debug.WriteLine($"=== FORM DEBUGGING ===");
            ...
            {
                if (key.Contains("ConfigGroups") || key.Contains("Options"))
                {
                    System.Diagnostics.Debug.WriteLine($"Form field: {key} = {Request.Form[key]}");
                }
            }
            
            // Debug received data
            System.Diagnostics.Debug.WriteLine($"Received ConfigGroups count: {viewModel.ConfigGroups?.Count ?? 0}");
            for (int i = 0; i < (viewModel.ConfigGroups?.Count ?? 0); i++)
            {
                var group = viewModel.ConfigGroups[i];
                System.Diagnostics.Debug.WriteLine($"Group {i}: {group.GroupName}, Options count: {group.Options?.Count ?? 0}");
                for (int j = 0; j < (group.Options?.Count ?? 0); j++)
                {
                    var option = group.Options[j];
                    System.Diagnostics.Debug.WriteLine($"  Option {j}: ProductID={option.ProductID}, VariantID={option.VariantID}");
                }
            }
            ...
            if (Request.HasFormContentType && Request.Form != null)
            {
                ...
                // Show some field names for debugging
                var configFields = Request.Form.Keys.Where(k => k.Contains("ConfigGroups")).Take(10);
                foreach (var field in configFields)
                {
                    System.Diagnostics.Debug.WriteLine($"  Field: {field} = {Request.Form[field]}");
                }
                if (Request.Form.Keys.Count(k => k.Contains("ConfigGroups")) > 10)
                {
                    System.Diagnostics.Debug.WriteLine($"  ... and {Request.Form.Keys.Count(k => k.Contains("ConfigGroups")) - 10} more ConfigGroups fields");
                }
            }
