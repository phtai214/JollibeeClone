...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, int? categoryId, bool? isAvailable, string sortOrder, int? page)
        {
            try
            {
                // Pagination settings
            ...
            catch (Exception ex)
            {
                ...
                return View(new PaginatedList<Product>(new List<Product>(), 0, 1, 12));
            }
        }
        ...
        public async Task<IActionResult> Create([Bind("ProductName,ShortDescription,Price,OriginalPrice,CategoryID,IsConfigurable,IsAvailable")] Product product, IFormFile? imageFile)
        {
            ...
            ModelState.Remove("Category"); // Also remove navigation property validation
            
            // Clear any potential validation issues related to Category
            var categoryKeys = ModelState.Keys.Where(k => k.Contains("Category", StringComparison.OrdinalIgnoreCase)).ToList();
            foreach (var key in categoryKeys)
            {
                ModelState.Remove(key);
            }
            
            // Debug: Log all ModelState errors before processing
            _logger.LogInformation("=== MODELSTATE AFTER CLEANUP ===");
            foreach (var key in ModelState.Keys)
            {
                var state = ModelState[key];
                if (state.Errors.Count > 0)
                {
                    _logger.LogWarning("Remaining ModelState error for {Key}: {Errors}", key, string.Join(", ", state.Errors.Select(e => e.ErrorMessage)));
                }
            }
            ...
            try
            {
                ...
                {
                    _logger.LogInformation("Processing image file: {Name}, Size: {Length} bytes, ContentType: {ContentType}", 
                    ...
                    else
                    {
                        try
                        {
                            ...
                            var savedFilePath = Path.Combine(_webHostEnvironment.WebRootPath, product.ImageUrl.TrimStart('/').Replace('/', Path.DirectorySeparatorChar));
                            var fileExists = System.IO.File.Exists(savedFilePath);
                            _logger.LogInformation("File exists on disk: {FileExists}, Path: {FilePath}", fileExists, savedFilePath);
                        }
                        catch (Exception ex)
                        {
                ...
                else
                {
                    ...
                    _logger.LogInformation("No image file provided (imageFile is {IsNull} with length {Length})", 
                        imageFile == null ? "null" : "not null", imageFile?.Length ?? 0);
                }

                // Validate remaining fields from ModelState (exclude removed fields)
                var excludedFields = new[] { "CategoryID", "ImageUrl", "Category" };
                foreach (var modelState in ModelState)
                {
                    // Skip any field containing "Category" to be absolutely sure
                    if (!excludedFields.Contains(modelState.Key) && 
                        !modelState.Key.Contains("Category", StringComparison.OrdinalIgnoreCase) &&
                        modelState.Value.Errors.Count > 0)
                    {
                        foreach (var error in modelState.Value.Errors)
                        {
                            if (!string.IsNullOrEmpty(error.ErrorMessage))
                            {
                                ...
                                isValid = false;
                                _logger.LogWarning("ModelState validation error for {Field}: {Error}", modelState.Key, error.ErrorMessage);
                            }
                        }
                    }
                }
                ...
                if (isValid && errors.Count == 0)
                {
                    ...
                    // Tự động tạo variants cho sản phẩm mới (chỉ tạo cho sản phẩm thường, không phải combo)
                    if (!product.IsConfigurable)
                    {
                        await CreateDefaultVariantsForProduct(product.ProductID);
                        _logger.LogInformation("Auto-created default variants for product: {ProductName} (ID: {ProductID})", 
        ...
        /// <summary>
        /// Tự động tạo 2 variants mặc định cho sản phẩm mới: "Vừa" và "Lớn"
        /// </summary>
        /// <param name="productId">ID của sản phẩm</param>
        private async Task CreateDefaultVariantsForProduct(int productId)
        {
            try
            {
                // Kiểm tra xem product đã có variants chưa
                ...
                if (!existingVariants)
                {
                    ...
                    _logger.LogInformation("Created 2 default variants for ProductID {ProductId}: Vừa (0đ), Lớn (+8,000đ)", productId);
                }
                else
                {
                    _logger.LogInformation("ProductID {ProductId} already has variants, skipping auto-creation", productId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error creating default variants for ProductID {ProductId}", productId);
                throw; // Re-throw để không làm lỗi việc tạo product
            }
        }
    }
}
