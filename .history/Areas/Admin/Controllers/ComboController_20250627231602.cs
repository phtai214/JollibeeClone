                ...
                try
                {
                    ...
                    foreach (var groupViewModel in viewModel.ConfigGroups)
                    {
                        ...
                        foreach (var optionViewModel in groupViewModel.Options)
                        {
                            ...
                            {
                                throw new InvalidOperationException($"Invalid ProductID in group '{groupViewModel.GroupName}' option {optionIndex}: {optionViewModel.ProductID}");
                            }
                            
                            if (!optionViewModel.VariantID.HasValue || optionViewModel.VariantID <= 0)
                            {
                                throw new InvalidOperationException($"Invalid VariantID in group '{groupViewModel.GroupName}' option {optionIndex}: {optionViewModel.VariantID} (ProductID: {optionViewModel.ProductID})");
                            }
