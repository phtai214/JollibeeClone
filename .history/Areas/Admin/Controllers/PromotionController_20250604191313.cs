...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class PromotionController : Controller
    {
        ...
        public async Task<IActionResult> Create(PromotionViewModel model)
        {
            try
            {
                _logger.LogInformation("=== SIMPLIFIED CREATE ===");
                ...
                // ONLY BASIC VALIDATION - NO COMPLEX RULES
                
                // Clear any model state errors we don't want
                ModelState.Clear();
                
                // Basic required validation
                if (string.IsNullOrWhiteSpace(model.PromotionName))
                {
                    ModelState.AddModelError(nameof(model.PromotionName), "Tên voucher là bắt buộc");
                }

                if (model.DiscountValue <= 0)
                {
                    ModelState.AddModelError(nameof(model.DiscountValue), "Giá trị giảm giá phải lớn hơn 0");
                }
                ...
                // FLEXIBLE SELECTION - ONLY NEED ONE OR THE OTHER
                if (!model.SelectedProductIds.Any() && !model.SelectedCategoryIds.Any())
                {
                    ModelState.AddModelError("", "Vui lòng chọn ít nhất MỘT sản phẩm HOẶC MỘT danh mục");
                }
                ...
                if (!string.IsNullOrEmpty(model.CouponCode))
                {
                    ...
                    if (existingCoupon)
                    {
                        ModelState.AddModelError(nameof(model.CouponCode), "Mã voucher đã tồn tại");
                    }
                }

                if (ModelState.IsValid)
                {
                    _logger.LogInformation("✅ Validation passed, creating promotion...");
                    ...
                    // Add product scopes if any selected
                    ...
                    // Add category scopes if any selected
                    ...
                    TempData["SuccessMessage"] = $"Voucher '{promotion.PromotionName}' đã được tạo thành công!";
                    return RedirectToAction(nameof(Index));
                }
                else
                {
        ...
        public async Task<IActionResult> Edit(int id, PromotionViewModel model)
        {
            ...
            try
            {
                ...
                if (!model.SelectedProductIds.Any() && !model.SelectedCategoryIds.Any())
                {
                    ModelState.AddModelError("", "Vui lòng chọn ít nhất một sản phẩm HOẶC một danh mục để áp dụng voucher. Bạn có thể chọn riêng sản phẩm, riêng danh mục, hoặc cả hai.");
                }
        ...
        public async Task<IActionResult> CreateSimple(PromotionViewModel model)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "Tên voucher là bắt buộc";
                    await LoadSelectLists(model);
                ...
                if (!model.SelectedProductIds.Any() && !model.SelectedCategoryIds.Any())
                {
                    TempData["ErrorMessage"] = "Vui lòng chọn ít nhất một sản phẩm hoặc danh mục";
                    await LoadSelectLists(model);
                    return View("Create", model);
                }
