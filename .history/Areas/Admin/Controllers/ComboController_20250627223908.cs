...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class ComboController : Controller
    {
        ...
        {
            // Debug form data
            Console.WriteLine($"ComboName: {viewModel.ComboName}");
            Console.WriteLine($"CategoryID: {viewModel.CategoryID}");
            Console.WriteLine($"ConfigGroups Count: {viewModel.ConfigGroups?.Count ?? 0}");
            ...
            if (viewModel.ConfigGroups != null)
            {
                for (int i = 0; i < viewModel.ConfigGroups.Count; i++)
                {
                    ...
                    Console.WriteLine($"Group {i}: {group.GroupName}, Options: {group.Options?.Count ?? 0}");
                    ...
                    if (group.Options != null)
                    {
                        for (int j = 0; j < group.Options.Count; j++)
                        {
                            ...
                            Console.WriteLine($"  Option {j}: ProductID={option.ProductID}, VariantID={option.VariantID}");
                        }
                    }
                }
            }
            ...
            if (!ModelState.IsValid)
            {
                ...
                {
                    Console.WriteLine($"ModelState Error - {error.Field}: {string.Join(", ", error.Errors)}");
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Console.WriteLine($"Exception in Create Combo: {ex.Message}");
