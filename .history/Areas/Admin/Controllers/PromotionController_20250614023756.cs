    ...
    public class PromotionController : Controller
    {
        ...
        public async Task<IActionResult> GetAvailableVouchersForUser(int userId)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "CÃ³ lá»—i xáº£y ra khi láº¥y danh sÃ¡ch voucher" });
            }
        }
        ...
        public async Task<IActionResult> CheckUserUsage(int userId, int promotionId)
        {
            try
            {
                ...
                return Json(new { 
                    success = true, 
                    hasUsed = hasUsed,
                    message = hasUsed ? "User Ä‘Ã£ sá»­ dá»¥ng voucher nÃ y" : "User chÆ°a sá»­ dá»¥ng voucher nÃ y"
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error checking user usage: user {UserId}, promotion {PromotionId}", 
                    userId, promotionId);
                return Json(new { success = false, message = "CÃ³ lá»—i xáº£y ra khi kiá»ƒm tra" });
            }
        }
