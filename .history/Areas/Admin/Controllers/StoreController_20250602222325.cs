...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class StoreController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("StoreName,StreetAddress,Ward,District,City,PhoneNumber,OpeningHours,Latitude,Longitude,ImageUrl,IsActive")] Store store, IFormFile? imageFile)
        {
            try
            {
                // Basic validation
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("StoreID,StoreName,StreetAddress,Ward,District,City,PhoneNumber,OpeningHours,Latitude,Longitude,ImageUrl,IsActive")] Store store, IFormFile? imageFile)
        {
            if (id != store.StoreID)
            {
                TempData["ErrorMessage"] = "ID cửa hàng không hợp lệ.";
