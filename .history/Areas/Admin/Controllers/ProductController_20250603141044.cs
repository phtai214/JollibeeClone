...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ProductName,ShortDescription,Price,OriginalPrice,ImageUrl,CategoryID,IsConfigurable,IsAvailable")] Product product, IFormFile? imageFile, string? selectedImageUrl)
        {
            ...
            _logger.LogInformation("- SelectedImageUrl parameter: {SelectedImageUrl}", selectedImageUrl ?? "NULL");
            ...
            // Log all parameters received by the action
            _logger.LogInformation("=== ACTION PARAMETERS ===");
            _logger.LogInformation("product parameter: {Product}", product != null ? "NOT NULL" : "NULL");
            _logger.LogInformation("imageFile parameter: {ImageFile}", imageFile != null ? $"NOT NULL ({imageFile.FileName})" : "NULL");
            _logger.LogInformation("selectedImageUrl parameter: {SelectedImageUrl}", selectedImageUrl ?? "NULL");

            // Remove all ModelState entries for CategoryID to start fresh
            ModelState.Remove("CategoryID");
            ModelState.Remove("ImageUrl"); // Remove ImageUrl validation since we handle it manually
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    {
                        _logger.LogInformation("Processing selected existing image: {SelectedImageUrl}", selectedImageUrl);
                        ...
                        _logger.LogInformation("Checking image file exists at: {ImagePath}", imagePath);
                        
                        if (System.IO.File.Exists(imagePath))
                        {
                            product.ImageUrl = selectedImageUrl;
                            _logger.LogInformation("✅ Selected image exists and set successfully: {ImageUrl}", product.ImageUrl);
                        }
                        else
                        {
                            _logger.LogWarning("❌ Selected image does not exist at path: {ImagePath}", imagePath);
                            
                            // Try with different path combinations
                            var alternativePath1 = Path.Combine(_webHostEnvironment.WebRootPath, "wwwroot", selectedImageUrl.TrimStart('/'));
                            var alternativePath2 = Path.Combine(_webHostEnvironment.WebRootPath, selectedImageUrl.TrimStart('/'));
                            
                            _logger.LogInformation("Trying alternative path 1: {Path}, Exists: {Exists}", alternativePath1, System.IO.File.Exists(alternativePath1));
                            _logger.LogInformation("Trying alternative path 2: {Path}, Exists: {Exists}", alternativePath2, System.IO.File.Exists(alternativePath2));
                            
                            if (System.IO.File.Exists(alternativePath2))
                            {
                                product.ImageUrl = selectedImageUrl;
                                _logger.LogInformation("✅ Found image at alternative path, using: {ImageUrl}", product.ImageUrl);
                            }
                            else
                            {
                                // Still set it anyway, maybe the path issue is elsewhere
                                product.ImageUrl = selectedImageUrl;
                                _logger.LogWarning("⚠️ Image not found but setting anyway: {ImageUrl}", product.ImageUrl);
                            }
                        }
                    }
                    else
                    {
                        // Không có ảnh nào được chọn - để ImageUrl = null (cho phép sản phẩm không có ảnh)
                        product.ImageUrl = null;
                        _logger.LogInformation("No image selected, product will be created without image");
                    }
                    ...
                    if (isValid)
                    {
                        ...
                        // Debug product data trước khi lưu
                        _logger.LogInformation("=== PRODUCT DATA BEFORE SAVE ===");
                        _logger.LogInformation("ProductName: {ProductName}", product.ProductName);
                        _logger.LogInformation("ShortDescription: {ShortDescription}", product.ShortDescription ?? "NULL");
                        _logger.LogInformation("Price: {Price}", product.Price);
                        _logger.LogInformation("OriginalPrice: {OriginalPrice}", product.OriginalPrice?.ToString() ?? "NULL");
                        _logger.LogInformation("CategoryID: {CategoryID}", product.CategoryID);
                        _logger.LogInformation("ImageUrl: {ImageUrl}", product.ImageUrl ?? "NULL");
                        _logger.LogInformation("IsConfigurable: {IsConfigurable}", product.IsConfigurable);
                        _logger.LogInformation("IsAvailable: {IsAvailable}", product.IsAvailable);
                        ...
                        // Verify sản phẩm đã được lưu đúng
                        var savedProduct = await _context.Products.FindAsync(product.ProductID);
                        if (savedProduct != null)
                        {
                            _logger.LogInformation("=== VERIFICATION: PRODUCT DATA FROM DATABASE ===");
                            _logger.LogInformation("DB ProductName: {ProductName}", savedProduct.ProductName);
                            _logger.LogInformation("DB ImageUrl: {ImageUrl}", savedProduct.ImageUrl ?? "NULL");
                            _logger.LogInformation("DB CategoryID: {CategoryID}", savedProduct.CategoryID);
                        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("ProductID,ProductName,ShortDescription,Price,OriginalPrice,ImageUrl,CategoryID,IsConfigurable,IsAvailable")] Product product, IFormFile? imageFile, string? selectedImageUrl)
        {
            ...
            // Clear any auto-generated ModelState errors for CategoryID
            ModelState.Remove("CategoryID");
            ...
            try
            {
                ...
                // Manual validation only - không dùng ModelState
                bool isValid = true;
                List<string> errors = new List<string>();
                ...
                // Validate category - Business logic validation
                ...
                if (isValid)
                {
                    _logger.LogInformation("Validation passed, proceeding with update...");
                    ...
                    else if (!string.IsNullOrEmpty(selectedImageUrl))
                    {
                        ...
                        if (System.IO.File.Exists(imagePath))
                        {
                            ...
                            if (!string.IsNullOrEmpty(existingProduct?.ImageUrl) && existingProduct.ImageUrl != selectedImageUrl)
                            {
                                DeleteImageFile(existingProduct.ImageUrl);
                            }
                ...
                // If validation failed, add errors to ModelState for display
                foreach (var error in errors)
                {
                    ModelState.AddModelError("", error);
                }
                _logger.LogWarning("Validation failed, returning to view with errors: {Errors}", string.Join(", ", errors));
            }
