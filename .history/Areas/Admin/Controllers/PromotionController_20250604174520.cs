...
namespace JollibeeClone.Areas.Admin.Controllers
{
    ...
    public class PromotionController : Controller
    {
        ...
        public async Task<IActionResult> DebugIndex()
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        debugInfo.Add("Database connection failed!");
                    }
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    var testQuery = await _context.Database.ExecuteSqlRawAsync("SELECT 1");
                    debugInfo.Add($"SQL execution test: Success");
                }
                catch (Exception ex)
                {
                    debugInfo.Add($"SQL execution test error: {ex.Message}");
                }
        ...
        // GET: Admin/Promotion/FixDatabase
        [HttpGet]
        [Route("Admin/Promotion/FixDatabase")]
        public async Task<IActionResult> FixDatabase()
        {
            try
            {
                ...
                // Check and create tables manually if they don't exist
                var createTablesScript = @"
                    ...
                    -- Create PromotionProductScopes table if not exists
                    ...
                    -- Create PromotionCategoryScopes table if not exists
                    ...
                    IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_PromotionProductScopes_Promotions_PromotionID')
                    BEGIN
                        ALTER TABLE [PromotionProductScopes] ADD CONSTRAINT [FK_PromotionProductScopes_Promotions_PromotionID] 
                        FOREIGN KEY ([PromotionID]) REFERENCES [Promotions] ([PromotionID]) ON DELETE CASCADE;
                    END

                    IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_PromotionProductScopes_Products_ProductID')
                    BEGIN
                        ALTER TABLE [PromotionProductScopes] ADD CONSTRAINT [FK_PromotionProductScopes_Products_ProductID] 
                        FOREIGN KEY ([ProductID]) REFERENCES [Products] ([ProductID]) ON DELETE CASCADE;
                    END

                    IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_PromotionCategoryScopes_Promotions_PromotionID')
                    BEGIN
                        ALTER TABLE [PromotionCategoryScopes] ADD CONSTRAINT [FK_PromotionCategoryScopes_Promotions_PromotionID] 
                        FOREIGN KEY ([PromotionID]) REFERENCES [Promotions] ([PromotionID]) ON DELETE CASCADE;
                    END

                    IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_PromotionCategoryScopes_Categories_CategoryID')
                    BEGIN
                        ALTER TABLE [PromotionCategoryScopes] ADD CONSTRAINT [FK_PromotionCategoryScopes_Categories_CategoryID] 
                        FOREIGN KEY ([CategoryID]) REFERENCES [Categories] ([CategoryID]) ON DELETE CASCADE;
                    END
                ";

                await _context.Database.ExecuteSqlRawAsync(createTablesScript);
                results.Add("✅ Database tables created/verified successfully");

                // Test tables
                try
                {
                    var promotionCount = await _context.Promotions.CountAsync();
                    results.Add($"✅ Promotions table: {promotionCount} records");
                }
                catch (Exception ex)
                {
                    results.Add($"❌ Promotions table error: {ex.Message}");
                }

                try
                {
                    var productScopeCount = await _context.PromotionProductScopes.CountAsync();
                    results.Add($"✅ PromotionProductScopes table: {productScopeCount} records");
                }
                catch (Exception ex)
                {
                    results.Add($"❌ PromotionProductScopes table error: {ex.Message}");
                }

                try
                {
                    var categoryScopeCount = await _context.PromotionCategoryScopes.CountAsync();
                    results.Add($"✅ PromotionCategoryScopes table: {categoryScopeCount} records");
                }
                catch (Exception ex)
                {
                    results.Add($"❌ PromotionCategoryScopes table error: {ex.Message}");
                }

                ViewBag.Results = results;
                TempData["SuccessMessage"] = "Database schema đã được sửa chữa thành công!";
                return View("FixDatabaseResult");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Lỗi khi sửa chữa database: {ex.Message}";
                return RedirectToAction("DebugIndex");
            }
        }
    }
