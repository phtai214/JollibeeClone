...
namespace JollibeeClone.Data
{
    public static class SeedAdminData
    {
        ...
        public static async Task SeedProductVariantsDirectAsync(AppDbContext context)
        {
            ...
            // Tạo biến thể cho tất cả sản phẩm bằng SQL (không dùng UNION để tránh duplicate)
            var sqls = new string[]
            {
                @"INSERT INTO ProductVariants (ProductID, VariantName, VariantType, PriceAdjustment, IsDefault, IsAvailable, DisplayOrder)
                  SELECT p.ProductID, 'vừa', 'Size', 0.00, 1, 1, 1
                  FROM Products p WHERE p.IsAvailable = 1",
                
                @"INSERT INTO ProductVariants (ProductID, VariantName, VariantType, PriceAdjustment, IsDefault, IsAvailable, DisplayOrder)
                  SELECT p.ProductID, 'lớn', 'Size', 10000.00, 0, 1, 2
                  FROM Products p WHERE p.IsAvailable = 1",
                
                @"INSERT INTO ProductVariants (ProductID, VariantName, VariantType, PriceAdjustment, IsDefault, IsAvailable, DisplayOrder)
                  SELECT p.ProductID, 'Thêm phô mai', 'Topping', 5000.00, 0, 1, 3
                  FROM Products p WHERE p.IsAvailable = 1 AND (LOWER(p.ProductName) LIKE '%burger%' OR LOWER(p.ProductName) LIKE '%bánh%')"
            };
